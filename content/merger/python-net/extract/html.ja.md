...
############################# Head ############################
head_title: "Python via .NETでHTMLページを抽出"
head_description: "GroupDocs.Merger for Python via .NETを使用してHTMLファイルから特定のページを迅速に抽出し、それらを別の文書として保存します。"

############################# Header ############################
title: "HTMLページの抽出" 
description: "GroupDocs.Merger for Python via .NETを使ってPythonアプリケーションの機能を向上させ、HTML文書のページ抽出をシームレスに提供します。"
...
steps:
    enable: true
    title: "HTMLページを抽出する方法"
    content: |
      [GroupDocs.Merger](/merger/python-net/)は、HTML文書からページを抽出するための方法を提供します。シームレスな文書処理をPython via .NETアプリケーションに統合してください。
      
      1. ソースHTML文書のファイルパスを指定します。
      2. 抽出したいページを選択します。
      3. 抽出プロセスを実行します。
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "文書から特定のページを抽出する"
      content: |
        この例では、HTMLファイルから選択したページ範囲を抽出し、それらを別の文書として保存する方法を示します。
      code:
        title: "JavaScript"
        content: |
          ```python {style=abap}
