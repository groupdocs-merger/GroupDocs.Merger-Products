...
head_title: "JavaでのXLSBページの交換と交換"
head_description: "ドキュメントのマージと分割APIを使用して、XLSBファイル内の2つのページの位置を交換および交換するJavaユーザー."

title: "JavaでのXLSBページの交換と交換"
description: "JavaおよびJ2SEアプリケーション用のファイルスプリッターAPIを使用した、XLSBファイル内の2ページのスワップと交換の位置."
...
steps:
    enable: true
    title_left: "XLSBファイルページをJavaで交換する"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、XLSBファイル内のページを簡単に交換、交換、および整理できます。
