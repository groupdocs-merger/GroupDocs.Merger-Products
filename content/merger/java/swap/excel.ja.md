...
############################# Head ############################
head_title: "JavaのEXCELページを交換して交換する"
head_description: "ドキュメント マージ API を使用して、Java の EXCEL ファイル内の 2 つのページの位置を入れ替えます。"

############################# Header ############################
title: "Java の EXCEL ページを入れ替える"
description: "EXCEL ページを数行の Java コードで入れ替えます。"
...
steps:
    enable: true
    title_left: "Java の EXCEL ファイル ページを交換"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) により、Java 開発者は、いくつかの簡単な手順を実装することで EXCEL ファイル内のページを簡単に交換できます.
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して EXCEL ファイル ページをスワップする方法">}}

        ```java    
        // GroupDocs.Merger API を使用して EXCEL ファイル ページを交換します
        int pageNumber1 = 6;
        int pageNumber2 = 1;
...
demos:
    enable: true
    title: "ライブ デモ - EXCEL ファイル ページをオンラインでスワップ"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/swap-pages/excel) Web サイトにアクセスして、EXCEL ファイル ページを今すぐスワップします。
       ライブデモには次の利点があります。
