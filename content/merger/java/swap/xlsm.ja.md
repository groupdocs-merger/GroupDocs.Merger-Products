...
############################# Head ############################
head_title: "JavaのXLSMページを交換して交換する"
head_description: "ドキュメント マージ API を使用して、Java の XLSM ファイル内の 2 つのページの位置を入れ替えます。"

############################# Header ############################
title: "Java の XLSM ページを入れ替える"
description: "XLSM ページを数行の Java コードで入れ替えます。"
...
steps:
    enable: true
    title_left: "Java の XLSM ファイル ページを交換"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) により、Java 開発者は、いくつかの簡単な手順を実装することで XLSM ファイル内のページを簡単に交換できます.
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して XLSM ファイル ページをスワップする方法">}}

        ```java    
        // GroupDocs.Merger API を使用して XLSM ファイル ページを交換します
        int pageNumber1 = 6;
        int pageNumber2 = 1;
...
demos:
    enable: true
    title: "ライブ デモ - XLSM ファイル ページをオンラインでスワップ"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/swap-pages/xlsm) Web サイトにアクセスして、XLSM ファイル ページを今すぐスワップします。
       ライブデモには次の利点があります。
