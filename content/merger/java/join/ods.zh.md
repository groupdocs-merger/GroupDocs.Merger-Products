...
############################# Head ############################
head_title: "通过 Java 和 J2SE 文档合并 API 加入 ODS 文件"
head_description: "使用文档合并 API 在 Java 中加入多个 ODS 文件，并将所有数据、样式和格式作为源文档。"

############################# Header ############################
title: "在 Java 中加入 ODS 个文件"
description: "用几行 Java 代码加入 ODS。"
...
about:
    ...
    content: |
        [GroupDocs.Merger for Java](/zh/merger/java/) 提供了一个方便的解决方案来加入多个 PDF、Microsoft Office（Word、Excel、PowerPoint、OneNote）、OpenDocument、HTML、图像和在 Java 应用程序中将许多其他文档合并到一个文件中。 GroupDocs.Merger 将为您节省大量精力，因为您可以加入 ODS 文档 - 无需安装任何第三方软件、桌面应用程序或插件。现在没有必要浪费您的时间和手动加入文件了！ GroupDocs 的使命是提供最佳质量并简化文档处理工作流程。
...
steps:
    enable: true
    title_left: "加入 Java 中的多个 ODS 文件"
    content_left: |
        [GroupDocs.Merger for Java](/zh/merger/java/) 通过执行几个简单的步骤，让 Java 开发人员可以轻松地加入多个 ODS 文件。
    ...
    code: |
     ...
     {{< merger/code-merger title="如何使用 Java 示例代码加入 ODS 文件">}}

        ```java    
        // 使用 GroupDocs.Merger for Java API 加入 ODS 文件
        // 使用输入 ODS 文档实例化合并
...
demos:
    ...
    content: |
       访问 [GroupDocs.Merger 现场演示](https://products.groupdocs.app/merger/ods) 网站，立即加入多个 ODS 文件。
       现场演示具有以下好处。
