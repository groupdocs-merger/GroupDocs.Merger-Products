...
############################# Head ############################
head_title: "Java の XPS ページを削除"
head_description: "ドキュメント マージ API を使用してページの順序を逆にすることにより、Java の XPS ファイルから単一のページまたはページのコレクションを削除または削除します。"

############################# Header ############################
title: "Java の XPS ページを削除"
description: "数行の Java コードを含む XPS ページを削除します。"
...
steps:
    enable: true
    title_left: "Java の XPS ファイル ページを削除"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) により、Java 開発者は XPS 内の単一または多数の特定のページを簡単に削除できますいくつかの簡単な手順を実装することにより、ファイルを作成します。
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して XPS ファイル ページを削除する方法">}}

        ```java    
        // GroupDocs.Merger API を使用して XPS ファイル ページを削除します
...
demos:
    enable: true
    title: "ライブデモ - XPS ページをオンラインで削除"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/remove-pages/xps) Web サイトにアクセスして、今すぐ XPS ファイル ページを削除してください。
       ライブデモには次の利点があります。
