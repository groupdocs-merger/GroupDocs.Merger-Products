...
head_title: "JavaでのXPSページの移動、並べ替え、並べ替え"
head_description: "ドキュメントマージAPIを使用してページの順序を逆にすることにより、XPSドキュメント内のページを移動、再配置、および並べ替えるJavaユーザー."

title: "JavaでXPSページを移動および並べ替える"
description: "JavaおよびJ2SEアプリケーション用のドキュメントマージAPIを使用して、XPSドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: "JavaでXPSファイルページを再配置する"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、XPSファイル内のページを簡単に移動および再配置できます。
    ...
    code: |
        ```java
        //GroupDocs.MergerAPIを使用してXPSファイルページを移動します
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ-XPSドキュメントページをオンラインに移動"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/xps)Webサイトにアクセスして、XPSファイル内のページを今すぐ移動します。
        ライブデモには次の利点があります
