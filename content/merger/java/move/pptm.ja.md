...
head_title: "JavaでのPPTMページの移動、並べ替え、並べ替え"
head_description: "Javaユーザーは、ドキュメントマージAPIを使用してページの順序を逆にすることにより、PPTMドキュメント内のページを移動、再配置、および並べ替えます。"

title: "JavaでのPPTMページの移動と並べ替え"
description: "JavaおよびJ2SEアプリケーション用のドキュメントマージAPIを使用して、PPTMドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: "JavaでPPTMファイルページを再配置する"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、PPTMファイル内のページを簡単に移動および再配置できます。
    ...
    code: |
        ```java
        //GroupDocs.MergerAPIを使用してPPTMファイルページを移動します
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ-PPTMドキュメントページをオンラインに移動"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/pptm)Webサイトにアクセスして、PPTMファイル内のページを今すぐ移動します。
        ライブデモには次の利点があります
