...
############################# Head ############################
head_title: "Java で ONE ページ移動"
head_description: "ドキュメント マージ API を使用して、Java 内の ONE ドキュメント内のページを任意の位置に移動します。"

############################# Header ############################
title: "Java 内の ONE ページを移動"
description: "数行の Java コードで ONE ページを移動します。"
...
steps:
    enable: true
    title_left: "Java で ONE ファイル ページを移動"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) は、Java 開発者が ONE ファイル内でページを簡単に移動できるようにするために、いくつかの簡単な手順を実装します。 .
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して ONE ファイル ページを移動する方法">}}

        ```java    
        // GroupDocs.Merger API を使用して ONE ファイル ページを移動する
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ - ONE ページをオンラインに移動"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/move-pages/one) Web サイトにアクセスして、今すぐ ONE ファイル ページを移動します。
       ライブデモには次の利点があります。
