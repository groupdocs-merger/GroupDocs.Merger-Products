...
head_title: "JavaでのEXCELページの移動、並べ替え、並べ替え"
head_description: "ドキュメントマージAPIを使用してページの順序を逆にすることにより、EXCELドキュメント内のページを移動、再配置、および並べ替えるJavaユーザー."

title: "JavaでのEXCELページの移動と並べ替え"
description: "JavaおよびJ2SEアプリケーション用のドキュメントマージAPIを使用して、EXCELドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: "JavaでEXCELファイルページを再配置する"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、EXCELファイル内のページを簡単に移動および再配置できます。
    ...
    code: |
        ```java
        //GroupDocs.MergerAPIを使用してEXCELファイルページを移動します
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ-EXCELドキュメントページをオンラインに移動"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/excel)Webサイトにアクセスして、今すぐEXCELファイル内のページを移動してください。
        ライブデモには次の利点があります
