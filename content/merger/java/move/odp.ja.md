...
############################# Head ############################
head_title: "Java で ODP ページ移動"
head_description: "ドキュメント マージ API を使用して、Java 内の ODP ドキュメント内のページを任意の位置に移動します。"

############################# Header ############################
title: "Java 内の ODP ページを移動"
description: "数行の Java コードで ODP ページを移動します。"
...
steps:
    enable: true
    title_left: "Java で ODP ファイル ページを移動"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) は、Java 開発者が ODP ファイル内でページを簡単に移動できるようにするために、いくつかの簡単な手順を実装します。 .
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して ODP ファイル ページを移動する方法">}}

        ```java    
        // GroupDocs.Merger API を使用して ODP ファイル ページを移動する
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ - ODP ページをオンラインに移動"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/move-pages/odp) Web サイトにアクセスして、今すぐ ODP ファイル ページを移動します。
       ライブデモには次の利点があります。
