...
############################# Head ############################
head_title: "DOCXをJavaで複数のファイルに分割"
head_description: "ドキュメント マージ API を使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて、1 つの DOCX ファイルを複数のファイルに分割します。"

############################# Header ############################
title: "DOCX Java のスプリッター"
description: "DOCX を数行の Java コードで分割します。"
...
steps:
    enable: true
    title_left: "Java で DOCX ファイルをページごとに分割"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) を使用すると、Java 開発者は、単一の DOCX ファイルを複数の結果ファイルに分割することが容易になります。いくつかの簡単な手順。
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して DOCX ファイルを分割する方法">}}

        ```java    
        // GroupDocs.Merger for Java API を使用して DOCX ファイルを分割します
...
demos:
    enable: true
    title: "ライブデモ - DOCX ファイルをオンラインで分割"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/docx) Web サイトにアクセスして、今すぐ DOCX ファイルを分割してください。
       ライブデモには次の利点があります。
