...
############################# Head ############################
head_title: "DOTMをJavaで複数のファイルに分割"
head_description: "ドキュメント マージ API を使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて、1 つの DOTM ファイルを複数のファイルに分割します。"

############################# Header ############################
title: "DOTM Java のスプリッター"
description: "DOTM を数行の Java コードで分割します。"
...
steps:
    enable: true
    title_left: "Java で DOTM ファイルをページごとに分割"
    content_left: |
        [GroupDocs.Merger for Java](/ja/merger/java/) を使用すると、Java 開発者は、単一の DOTM ファイルを複数の結果ファイルに分割することが容易になります。いくつかの簡単な手順。
    ...
    code: |
     ...
     {{< merger/code-merger title="Java サンプル コードを使用して DOTM ファイルを分割する方法">}}

        ```java    
        // GroupDocs.Merger for Java API を使用して DOTM ファイルを分割します
...
demos:
    enable: true
    title: "ライブデモ - DOTM ファイルをオンラインで分割"
    content: |
       [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/dotm) Web サイトにアクセスして、今すぐ DOTM ファイルを分割してください。
       ライブデモには次の利点があります。
