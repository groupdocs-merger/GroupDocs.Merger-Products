...
head_title: "JavaでOTTを複数のファイルに抽出する"
head_description: "Javaユーザーは、ドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて単一のOTTファイルを複数のファイルに抽出します."

title: "JavaでOTTを複数のファイルに抽出する"
description: "JavaおよびJ2SEアプリケーション用のドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて、単一のOTTファイルを複数の結果ファイルに抽出します。"
...
steps:
    enable: true
    title_left: "JavaでページごとにOTTファイルを抽出する"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、単一のOTTファイルを複数の結果ファイルに簡単に抽出できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してOTTファイルを抽出します
...
demos:
    enable: true
    title: "ライブデモ-OTTページをオンラインで実行"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/ott)Webサイトにアクセスして、今すぐOTTファイルを抽出します。
        ライブデモには次の利点があります
