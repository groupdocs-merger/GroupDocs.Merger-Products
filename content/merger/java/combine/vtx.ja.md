...
head_title: "JavaとJ2SEDocumentsMergerAPIを介してVTXファイルを結合する"
head_description: "ドキュメントマージAPIを使用して、Javaで複数のVTXファイルを結合し、すべてのデータ、スタイル、およびフォーマットをソースドキュメントとして使用します."

title: "JavaアプリケーションでVTXファイルを組み合わせる"
description: "JavaドキュメントマージAPIを使用して、複数のVTXファイルを1つのファイルに結合します。選択したページまたはさまざまなソースドキュメントのページ範囲を、すべてのデータ、スタイル、およびフォーマットをソースドキュメントとして持つ単一の結果ドキュメントに結合します."
...
steps:
    enable: true
    title_left: "Javaで複数のVTXファイルを組み合わせる"
    content_left: |
        [GroupDocs.Merger](/merger/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、複数のVTXファイルを簡単に組み合わせることができます。
    ...
    code: |
        ```java
        // GroupDocs.Merger for Java APIを使用してVTXファイルを結合します
        //入力VTXドキュメントを使用して合併をインスタンス化します
...
demos:
    ...
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/family)Webサイトにアクセスして、現在複数のVTXファイルを結合してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-alt"
          title: "VTXファイル形式について"
          content: |
            VTX拡張子の付いたファイルは、XMLファイル形式でディスクに保存されるMicrosoftVisioの描画テンプレートです。このテンプレートは、同じ設定の複数のVisioファイルを作成するために使用できる基本設定のファイルを提供することを目的としています。もう1つの同様の形式は、XMLではなくバイナリ形式で保存されるVSTです。 VTXファイルは、Visio2010以降のバージョンでサポートされています。
