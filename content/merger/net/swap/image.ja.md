...
head_title: "C＃.NETでの画像ページの交換と交換"
head_description: " C＃.NETユーザーは、ファイル分割APIを使用してIMAGEファイル内の2ページの位置を交換および交換します."

title: ".NETで画像ページを入れ替える"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のファイルスプリッターAPIを使用して、IMAGEファイル内の2つのページの位置を交換および交換します."
...
steps:
    enable: true
    title_left: ".NETで画像ファイルページを入れ替える"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、いくつかの簡単な手順を実行することで、.NET開発者がIMAGEファイル内のページを簡単に交換、交換、整理できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してIMAGEファイルページを交換します
        int pageNumber1 = 3;
        int pageNumber2 = 6;
...
demos:
    enable: true
    title: "ライブデモ-画像ファイルページをオンラインで交換"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/swap-pages/image)Webサイトにアクセスして、IMAGEファイル内のページを今すぐ交換してください。
        ライブデモには次の利点があります
