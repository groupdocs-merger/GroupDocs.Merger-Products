...
############################# Head ############################
head_title: "C#のVSTXページを交換して交換する"
head_description: "ドキュメント マージ API を使用して、C# の VSTX ファイル内の 2 つのページの位置を入れ替えます。"

############################# Header ############################
title: "C# の VSTX ページを入れ替える"
description: "VSTX ページを数行の .NET コードで入れ替えます。"
...
steps:
    enable: true
    title_left: ".NET の VSTX ファイル ページを交換"
    content_left: |
        [GroupDocs.Merger for .NET](/ja/merger/net/) により、C# 開発者は、いくつかの簡単な手順を実装することで VSTX ファイル内のページを簡単に交換できます.
    ...
    code: |
     ...
     {{< merger/code-merger title="C# サンプル コードを使用して VSTX ファイル ページをスワップする方法">}}
