...
head_title: "C＃.NETでのVSDXページの交換と交換"
head_description: " C＃.NETユーザーは、ファイル分割APIを使用してVSDXファイル内の2つのページの位置を交換および交換します."

title: ".NETでVSDXページを交換する"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のファイルスプリッターAPIを使用して、VSDXファイル内の2つのページの位置を交換および交換します."
...
steps:
    enable: true
    title_left: ".NETでVSDXファイルページを交換する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、VSDXファイル内のページを簡単に交換、交換、および整理できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してVSDXファイルページを交換します
        int pageNumber1 = 3;
        int pageNumber2 = 6;
...
demos:
    enable: true
    title: "ライブデモ-VSDXファイルページをオンラインで交換"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/swap-pages/vsdx)Webサイトにアクセスして、VSDXファイル内のページを今すぐ交換してください。
        ライブデモには次の利点があります
