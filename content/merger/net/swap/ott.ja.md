...
head_title: "C＃.NETでのOTTページの交換と交換"
head_description: " C＃.NETユーザーは、ファイル分割APIを使用してOTTファイル内の2ページの位置を交換および交換します."

title: ".NETでOTTページを交換する"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のファイルスプリッターAPIを使用して、OTTファイル内の2つのページの位置を交換および交換します."
...
steps:
    enable: true
    title_left: ".NETでOTTファイルページを交換する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、いくつかの簡単な手順を実行することで、.NET開発者がOTTファイル内のページを簡単に交換、交換、整理できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してOTTファイルページを交換します
        int pageNumber1 = 3;
        int pageNumber2 = 6;
...
demos:
    enable: true
    title: "ライブデモ-OTTファイルページをオンラインで交換"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/swap-pages/ott)Webサイトにアクセスして、OTTファイル内のページを今すぐ交換してください。
        ライブデモには次の利点があります
