...
############################# Head ############################
head_title: "C#のXPSページを交換して交換する"
head_description: "ドキュメント マージ API を使用して、C# の XPS ファイル内の 2 つのページの位置を入れ替えます。"

############################# Header ############################
title: "C# の XPS ページを入れ替える"
description: "XPS ページを数行の .NET コードで入れ替えます。"
...
steps:
    enable: true
    title_left: ".NET の XPS ファイル ページを交換"
    content_left: |
        [GroupDocs.Merger for .NET](/ja/merger/net/) により、C# 開発者は、いくつかの簡単な手順を実装することで XPS ファイル内のページを簡単に交換できます.
    ...
    code: |
     ...
     {{< merger/code-merger title="C# サンプル コードを使用して XPS ファイル ページをスワップする方法">}}
