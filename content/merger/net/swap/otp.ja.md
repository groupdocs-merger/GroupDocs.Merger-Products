...
############################# Head ############################
head_title: "C#のOTPページを交換して交換する"
head_description: "ドキュメント マージ API を使用して、C# の OTP ファイル内の 2 つのページの位置を入れ替えます。"

############################# Header ############################
title: "C# の OTP ページを入れ替える"
description: "OTP ページを数行の .NET コードで入れ替えます。"
...
steps:
    enable: true
    title_left: ".NET の OTP ファイル ページを交換"
    content_left: |
        [GroupDocs.Merger for .NET](/ja/merger/net/) により、C# 開発者は、いくつかの簡単な手順を実装することで OTP ファイル内のページを簡単に交換できます.
    ...
    code: |
     ...
     {{< merger/code-merger title="C# サンプル コードを使用して OTP ファイル ページをスワップする方法">}}
