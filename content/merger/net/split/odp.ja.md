...
head_title: "C＃.NETでODPを複数のファイルに分割する"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて単一のODPファイルを複数のファイルに分割します."

title: ".NETでODPを複数のファイルに分割"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて、単一のODPファイルを複数の結果ファイルに分割します。"
...
steps:
    enable: true
    title_left: ".NETでODPファイルページを分割する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、単一のODPファイルを複数の結果ファイルに簡単に分割できます。
...
demos:
    ...
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/odp)Webサイトにアクセスして、ODPファイルを今すぐ分割します。
        ライブデモには次の利点があります
