...
head_title: "C＃.NETでCSVを複数のファイルに分割"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて単一のCSVファイルを複数のファイルに分割します."

title: "CSVを.NETで複数のファイルに分割"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、ページ番号、ページ間隔、偶数ページまたは奇数ページに基づいて、単一のCSVファイルを複数の結果ファイルに分割します。"
...
steps:
    enable: true
    title_left: ".NETでCSVファイルページを分割する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、単一のCSVファイルを複数の結果ファイルに簡単に分割できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してCSVファイルを分割します
...
demos:
    enable: true
    title: "ライブデモ-CSVファイルをオンラインで分割"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/csv)Webサイトにアクセスして、CSVファイルを今すぐ分割します。
        ライブデモには次の利点があります
