...
head_title: "C＃でMHTMLファイルをマージする| MHTMLの合併"
head_description: "C＃/。NETマージAPIを使用して、複数のMHTMLファイルを1つのファイルにマージします。特定のページまたはページ範囲をさまざまなドキュメントから単一のドキュメントにマージします."

title: "C＃でMHTMLファイルをマージする"
description: "C＃マージAPIを使用して、複数のMHTMLファイルを1つのファイルにマージします。選択したページまたはページ範囲をさまざまなソースドキュメントから単一の結果ドキュメントにマージします."
...
steps:
    enable: true
    title_left: "複数のMHTMLファイルをマージする方法"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内で2つ以上のMHTMLファイルを簡単にマージできます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してMHTMLファイルをマージします
