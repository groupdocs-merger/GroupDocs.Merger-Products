...
############################# Head ############################
head_title: "Scal VDX pliki w C# | VDX Połączenie"
head_description: "Połącz wiele plików VDX w jeden plik za pomocą interfejsu API łączenia dokumentów C# .NET. Scal określone strony lub zakresy stron z różnych dokumentów w jeden dokument."
...
about:
    ...
    content: |
        [GroupDocs.Merger for .NET](/pl/merger/net/) zapewnia wygodne rozwiązanie do scalania wielu plików PDF, pakietu Microsoft Office (Word, Excel, PowerPoint, OneNote), OpenDocument, HTML, obrazów i wiele innych dokumentów w jednym pliku w aplikacjach .NET. GroupDocs.Merger zaoszczędzi Ci wiele wysiłku, ponieważ możesz łączyć dokumenty VDX - nie ma potrzeby instalowania żadnego oprogramowania innych firm, aplikacji komputerowych ani wtyczek. Teraz nie musisz tracić czasu i ręcznie scalać pliki! Misją GroupDocs jest zapewnienie najwyższej jakości i uproszczenie przepływów pracy przy przetwarzaniu dokumentów.
...
steps:
    ...
    content_left: |
        [GroupDocs.Merger for .NET](/pl/merger/net/) ułatwia programistom .NET scalanie dwóch lub więcej plików VDX w ich aplikacjach przez zaimplementowanie kilka prostych kroków.
    ...
    code: |
     ...
     {{< merger/code-merger title="Jak scalić pliki VDX przy użyciu przykładowego kodu C#">}}

        ```csharp    
        // Scal pliki VDX za pomocą GroupDocs.Merger API
