...
############################# Head ############################
head_title: "HTMLページの抽出 - .NET"
head_description: "GroupDocs.Merger for .NETを使用して、HTMLファイルから特定のページを迅速に抽出し、別の文書として保存します。"

############################# Header ############################
title: "HTMLページの抽出" 
description: "GroupDocs.Merger for .NETを.NETアプリケーションに統合し、高度なページ抽出機能を使用してHTMLファイルを効率的に処理します。"
...
steps:
    ...
    title: "HTMLページの抽出方法"
    content: |
      [GroupDocs.Merger](/merger/net/)を使用すると、HTML文書からページを抽出するのが簡単です。シームレスな文書処理機能を利用して、.NETアプリケーションを強化しましょう。
      
      1. ソースのHTML文書のファイルパスを提供します。
      2. 抽出したいページを選択します。
      3. 抽出プロセスを実行します。
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "文書から特定のページを抽出"
      content: |
        この例では、HTMLファイルから選択されたページ範囲を抽出し、それを新しい文書として保存する方法を示します。
      code:
        title: "C#"
        content: |
          ```csharp {style=abap}
