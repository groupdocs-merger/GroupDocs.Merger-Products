...
head_title: "C＃ASP.NETおよびVB.NETでMHTMLページを抽出する"
head_description: "C＃.NETのMHTMLファイルからページをすばやく抽出します。ドキュメントマージAPIを使用して、選択したページを含む新しいドキュメントを保存します."

title: "C＃.NETアプリケーションでMHTMLページを抽出する"
description: "選択したページをMHTMLファイルからすばやく抽出し、.NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して新しいドキュメントとして保存します。"
...
steps:
    enable: true
    title_left: "C＃.NETのMHTMLファイルからページを抽出する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、MHTMLファイルから目的のページを簡単に抽出し、選択したページを含む新しいファイルとして保存できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してMHTMLファイルページを抽出します
...
demos:
    enable: true
    title: "ライブデモ-ExractMHTMLDocument Pages App"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/mhtml)Webサイトにアクセスして、MHTMLファイルページを今すぐ抽出します。
        ライブデモには次の利点があります
