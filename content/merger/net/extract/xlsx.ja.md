...
head_title: "C＃ASP.NETおよびVB.NETでXLSXページを抽出する"
head_description: "C＃.NETのXLSXファイルからページをすばやく抽出します。ドキュメントマージAPIを使用して、選択したページを含む新しいドキュメントを保存します."

title: "C＃.NETアプリケーションでXLSXページを抽出する"
description: "XLSXファイルから選択したページをすばやく抽出し、.NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して新しいドキュメントとして保存します."
...
steps:
    enable: true
    title_left: "C＃.NETでXLSXファイルからページを抽出する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、XLSXファイルから目的のページを簡単に抽出し、選択したページを含む新しいファイルとして保存できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してXLSXファイルページを抽出します
...
demos:
    enable: true
    title: "ライブデモ-ExractXLSXDocument Pages App"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/xlsx)Webサイトにアクセスして、XLSXファイルページを今すぐ抽出します。
        ライブデモには次の利点があります
