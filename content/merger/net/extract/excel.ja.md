...
############################# Head ############################
head_title: "EXCELページの抽出 - .NET"
head_description: "GroupDocs.Merger for .NETを使用して、EXCELファイルから特定のページを迅速に抽出し、別の文書として保存します。"

############################# Header ############################
title: "EXCELページの抽出" 
description: "GroupDocs.Merger for .NETを.NETアプリケーションに統合し、高度なページ抽出機能を使用してEXCELファイルを効率的に処理します。"
...
steps:
    ...
    title: "EXCELページの抽出方法"
    content: |
      [GroupDocs.Merger](/merger/net/)を使用すると、EXCEL文書からページを抽出するのが簡単です。シームレスな文書処理機能を利用して、.NETアプリケーションを強化しましょう。
      
      1. ソースのEXCEL文書のファイルパスを提供します。
      2. 抽出したいページを選択します。
      3. 抽出プロセスを実行します。
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "文書から特定のページを抽出"
      content: |
        この例では、EXCELファイルから選択されたページ範囲を抽出し、それを新しい文書として保存する方法を示します。
      code:
        title: "C#"
        content: |
          ```csharp {style=abap}
