...
head_title: "C＃ASP.NETおよびVB.NETでEXCELページを抽出する"
head_description: "C＃.NETのEXCELファイルからページをすばやく抽出します。ドキュメントマージAPIを使用して、選択したページを含む新しいドキュメントを保存します."

title: "C＃.NETアプリケーションでEXCELページを抽出する"
description: "選択したページをEXCELファイルからすばやく抽出し、.NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して新しいドキュメントとして保存します。"
...
steps:
    enable: true
    title_left: "C＃.NETのEXCELファイルからページを抽出する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、EXCELファイルから目的のページを簡単に抽出し、選択したページを含む新しいファイルとして保存できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してEXCELファイルページを抽出します
        ...
        //入力EXCELドキュメントを使用してマージをインスタンス化します
...
demos:
    enable: true
    title: "ライブデモ-ExractEXCELDocument Pages App"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/doc)Webサイトにアクセスして、今すぐEXCELファイルページを抽出してください。
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-excel"
          ...
          content: |
            スプレッドシートファイルには、行と列の形式のデータが含まれています。 WindowsとMacOSオペレーティングシステムの両方で利用できるようになったMicrosoftExcelなどのスプレッドシートソフトウェアアプリケーションを使用して、このようなファイルを開いたり、表示したり、編集したりできます。同様に、Googleスプレッドシートは、どのWebブラウザからでも機能する無料のオンラインスプレッドシート作成および編集ツールです。
