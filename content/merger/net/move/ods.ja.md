...
head_title: "C＃.NETでのODSページの移動、並べ替え、並べ替え"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用してページの順序を逆にすることにより、ODSドキュメント内のページを移動、再配置、および並べ替えます."

title: ".NETでのODSページの移動と並べ替え"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、ODSドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: ".NETでODSファイルページを移動する"
    content_left: |
