...
head_title: "C＃.NETでXPSページを移動、並べ替え、並べ替える"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用してページの順序を逆にすることにより、XPSドキュメント内のページを移動、並べ替え、および並べ替えます."

title: ".NETでのXPSページの移動と並べ替え"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、XPSドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: ".NETでXPSファイルページを移動する"
    content_left: |
