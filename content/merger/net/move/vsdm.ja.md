...
head_title: "C＃.NETでのVSDMページの移動、並べ替え、並べ替え"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用してページの順序を逆にすることにより、VSDMドキュメント内のページを移動、再配置、および並べ替えます."

title: ".NETでのVSDMページの移動と並べ替え"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、VSDMドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: ".NETでVSDMファイルページを移動する"
    content_left: |
