...
head_title: "C＃.NETでのDOTMページの移動、並べ替え、並べ替え"
head_description: "C＃.NETユーザーは、ドキュメントマージAPIを使用してページの順序を逆にすることにより、DOTMドキュメント内のページを移動、再配置、および並べ替えます."

title: ".NETでのDOTMページの移動と並べ替え"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントマージAPIを使用して、DOTMドキュメント内のページを任意の位置に移動および再配置します."
...
steps:
    enable: true
    title_left: ".NETでDOTMファイルページを移動する"
    content_left: |
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してDOTMファイルページを移動します
        int pageNumber = 6;
        int newPageNumber = 1;
...
demos:
    enable: true
    title: "ライブデモ-DOTMドキュメントページをオンラインに移動"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/dotm)Webサイトにアクセスして、今すぐDOTMファイル内のページを移動してください。
        ライブデモには次の利点があります
