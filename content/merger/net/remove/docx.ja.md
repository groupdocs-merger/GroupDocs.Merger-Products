...
head_title: "C＃.NETでDOCXページを削除する| DOCXファイルからページを削除する"
head_description: "C＃.NETユーザーは、ドキュメントのマージと分割APIを使用してDOCXファイルから単一のページまたはページのコレクションを削除または削除します."

title: ".NETのDOCXからページを削除または削除する"
description: "ドキュメントのマージと分割APIfor.NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーションを使用して、DOCXファイルから単一のページまたはページのコレクションを削除または削除します."
...
steps:
    enable: true
    title_left: ".NETでDOCXファイルページを削除する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、DOCXファイル内の1つまたは複数のページを簡単に削除できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してDOCXファイルページを削除します
...
demos:
    enable: true
    title: "ライブデモ-DOCXドキュメントページをオンラインで削除"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/remove-pages/docx)Webサイトにアクセスして、DOCXファイル内のページを今すぐ削除してください。
        ライブデモには次の利点があります
