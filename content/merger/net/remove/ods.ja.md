...
head_title: "C＃.NETでODSページを削除する| ODSファイルからページを削除する"
head_description: "C＃.NETユーザーは、ドキュメントのマージと分割APIを使用して、ODSファイルから単一のページまたはページのコレクションを削除または削除します."

title: ".NETのODSからページを削除または削除する"
description: "ドキュメントのマージと分割APIfor.NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーションを使用して、ODSファイルから単一のページまたはページのコレクションを削除または削除します."
...
steps:
    enable: true
    title_left: ".NETでODSファイルページを削除する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、ODSファイル内の1つまたは複数のページを簡単に削除できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してODSファイルページを削除します
...
demos:
    enable: true
    title: "ライブデモ-ODSドキュメントページをオンラインで削除"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/remove-pages/ods)Webサイトにアクセスして、ODSファイル内のページを今すぐ削除してください。
        ライブデモには次の利点があります
