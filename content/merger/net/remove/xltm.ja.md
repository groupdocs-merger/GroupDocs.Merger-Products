...
head_title: "C＃.NETでXLTMページを削除する| XLTMファイルからページを削除する"
head_description: "ドキュメントのマージと分割APIを使用してXLTMファイルから単一のページまたはページのコレクションを削除または削除するC＃.NETユーザー."

title: ".NETのXLTMからページを削除または削除する"
description: ".NET（C＃、ASP.NET、VB.NET、.NET Core）アプリケーション用のドキュメントのマージおよび分割APIを使用して、XLTMファイルから単一のページまたはページのコレクションを削除または削除します."
...
steps:
    enable: true
    title_left: ".NETでXLTMファイルページを削除する"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、XLTMファイル内の1つまたは複数のページを簡単に削除できます。
    ...
    code: |
        ```cs
        //GroupDocs.MergerAPIを使用してXLTMファイルページを削除します
...
demos:
    enable: true
    title: "ライブデモ-XLTMドキュメントページをオンラインで削除"
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/splitter/remove-pages/xltm)Webサイトにアクセスして、XLTMファイル内のページを今すぐ削除してください。
        ライブデモには次の利点があります
