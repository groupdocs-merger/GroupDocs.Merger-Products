...
############################# Head ############################
head_title: "Liity CSV tiedostoihin C# | CSV Sulautuminen"
head_description: "Yhdistä useita CSV-tiedostoja yhdeksi tiedostoksi käyttämällä C# .NET -asiakirjojen yhdistämissovellusliittymää. Yhdistä tietyt sivut tai sivualueet useista asiakirjoista yhdeksi asiakirjaksi."

############################# Header ############################
title: "Liity CSV tiedostoon C#"
description: "Liity kohtaan CSV muutamalla rivillä .NET-koodia."
...
about:
    ...
    content: |
        [GroupDocs.Merger for .NET](/fi/merger/net/) tarjoaa kätevän ratkaisun useiden PDF-, Microsoft Office- (Word, Excel, PowerPoint, OneNote), OpenDocument-, HTML-, kuvien ja monet muut asiakirjat yhdeksi tiedostoksi .NET sovelluksissa. GroupDocs.Merger säästää paljon vaivaa, koska voit liittyä CSV asiakirjoihin - sinun ei tarvitse asentaa kolmannen osapuolen ohjelmistoja, työpöytäsovelluksia tai laajennuksia. Nyt on turhaa tuhlata aikaasi ja yhdistää tiedostoja manuaalisesti! GroupDocsin tehtävänä on tarjota parasta laatua ja yksinkertaistaa asiakirjankäsittelyn työnkulkuja.
...
steps:
    enable: true
    title_left: "Kuinka yhdistää useita CSV-tiedostoja"
    content_left: |
        [GroupDocs.Merger for .NET](/fi/merger/net/) helpottaa tuotteen .NET kehittäjien yhdistämistä kahteen tai useampaan CSV tiedostoon sovelluksissaan ottamalla käyttöön muutama helppo askel.
    ...
    code: |
     ...
     {{< merger/code-merger title="Kuinka liittää CSV tiedostot käyttämällä C# esimerkkikoodia">}}

        ```csharp    
        // Liity CSV tiedostoon GroupDocs.Merger API:n avulla
        // Toteuta yhdistäminen syötteellä CSV
...
demos:
    ...
    content: |
       Liity useampaan kuin yhteen CSV tiedostoon juuri nyt käymällä [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/csv) -sivustolla.
