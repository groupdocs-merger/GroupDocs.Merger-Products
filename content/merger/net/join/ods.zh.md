...
############################# Head ############################
head_title: "加入 C# 中的 ODS 个文件 | ODS 合并"
head_description: "使用 C# .NET 文档合并 API 将多个 ODS 文件合并到一个文件中。将特定页面或页面范围从各种文档连接到单个文档。"

############################# Header ############################
title: "在 C# 中加入 ODS 个文件"
description: "用几行 .NET 代码加入 ODS。"
...
about:
    ...
    content: |
        [GroupDocs.Merger for .NET](/zh/merger/net/) 提供了一个方便的解决方案来加入多个 PDF、Microsoft Office（Word、Excel、PowerPoint、OneNote）、OpenDocument、HTML、图像和在 .NET 应用程序中将许多其他文档合并到一个文件中。 GroupDocs.Merger 将为您节省大量精力，因为您可以加入 ODS 文档 - 无需安装任何第三方软件、桌面应用程序或插件。现在没有必要浪费您的时间和手动加入文件了！ GroupDocs 的使命是提供最佳质量并简化文档处理工作流程。
...
steps:
    ...
    code: |
     ...
     {{< merger/code-merger title="如何使用 C# 示例代码加入 ODS 文件">}}

        ```csharp    
        // 使用 GroupDocs.Merger API 加入 ODS 文件
        // 使用输入 ODS 文档实例化合并
