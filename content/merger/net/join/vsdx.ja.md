...
head_title: "VSDXファイルをC＃（.NET Core）で1つに結合-VSDXマージャー"
head_description: "C＃.NETドキュメントマージAPIを使用して、複数のVSDXファイルを1つのファイルに結合します。特定のページまたはページ範囲をさまざまなドキュメントから単一のドキュメントに結合する."

title: "C＃（.NET Core）でVSDXファイルに参加する"
description: "C＃.NETドキュメントマージAPIを使用して、複数のVSDXファイルを1つのファイルに結合します。選択したページまたはページ範囲をさまざまなソースドキュメントから単一の結果ドキュメントに結合します."
...
steps:
    enable: true
    title_left: "複数のVSDXファイルを結合する方法"
    content_left: |
        [GroupDocs.Merger](/merger/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実行することで、アプリケーション内で2つ以上のVSDXファイルを簡単に結合できます。
    ...
    code: |
        ```cs
        // GroupDocs.Merger for .NET APIを使用してVSDXファイルを結合します
...
demos:
    ...
    content: |
        [GroupDocs.Merger Live Demos](https://products.groupdocs.app/merger/total)Webサイトにアクセスして、今すぐ複数のVSDXファイルに参加してください。  
        ライブデモには次の利点があります
